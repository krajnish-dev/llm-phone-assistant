...
class UpdateDeliveryDateTool extends StructuredTool {
    ...
    schema = z.object({
        phoneNumber: z.string().describe("The phone number associated with the order"),
        orderedItem: z.string().describe("The name of the ordered item"),
        expectedDeliveryDate: z.string().describe("The new expected delivery date in YYYY-MM-DD format"),
    });

    async _call({ phoneNumber, orderedItem, expectedDeliveryDate }) {
        try {
            const headers = {
            ...
            const payload = {
                phoneNumber,
                orderedItem,
                updatedDeliveryDate: expectedDeliveryDate,
            };
